# Taken from https://github.com/lindsayhrlee/SnapHiC-D/blob/main/snapHiC_diff.py
import numpy as np
import pandas as pd
import qnorm
...
def rest_of_steps(GAP, chr_list, dir_in_B, dir_in_A, ann_dir, out_dir, b_ID, gID, BINSIZE,fdr_t):
    ...
    for ID in range(0, len(ann)):
        if(ann['group'][ID] == "A"):
            ...
            x = cool_to_coo(dir_in_A + ann['name'][ID], CHR ,BINSIZE)
        else:
            #x = pd.read_csv(dir_in_B + ann['name'][ID], header=None, sep="\t")
            x = cool_to_coo(dir_in_B + ann['name'][ID], CHR ,BINSIZE)
